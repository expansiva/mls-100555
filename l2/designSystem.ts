/// <mls fileReference="_100555_/l2/designSystem.ts"  enhancement="_blank" />
 ...
 * (MANDATORY_COLOR_ROLES / DEFAULT_TOKENS_TEMPLATE). See TASK-100555-ds-tokens-por-papel.md.
 *
 * Naming rules (color):
 ...
 *
 * The palette is the same one _100554_ carries, and the two entries were kept
 * IDENTICAL on purpose: both projects render side by side in the studio, so a
 * different surface grey between them would show up as a seam.
 *
 * Where the values came from:
 *  - page-bg / surface-bg both inherit bg-primary-color (#ffffff / #0d1117):
 *    this project paints the page and its panels with the same colour, and the
 *    old model had no separate page token.
 ...
 *    (the 14 box-shadow + 9 border rules); the -hover/-focus/-disabled steps are
 *    derived, since the old grey scale had no states.
 *  - button-primary-bg is active-color (#1890FF): in this project active-color
 *    is overwhelmingly a button fill (28 of its 30 background rules are literal
 *    `button { }` blocks, with an -hover twin). Its 8 text rules read as
 *    selection/focus, so it also feeds selected-text, selected-border and
 *    focus-ring. info-color paints one more button (button-primary-bg) and
 *    otherwise keeps its state reading (status-info-text).
