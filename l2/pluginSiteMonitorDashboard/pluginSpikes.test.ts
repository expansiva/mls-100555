/// <mls fileReference="_100555_/l2/pluginSiteMonitorDashboard/pluginSpikes.test.ts" enhancement="_blank" />

import { IPluginTestCase, mount, cleanup, compare, query, mountAndVerify } from '/_102027_/l2/plugins/pluginTestUtils.js';
import { pluginData, PluginSpikes } from '/_100555_/l2/pluginSiteMonitorDashboard/pluginSpikes.js';

import '/_100555_/l2/pluginSiteMonitorDashboard/pluginSpikes.js'; // side-effect import: guarantees this module's top-level registration (e.g. @customElement) always runs, regardless of whether the named imports above survive compilation (TS elides imports used only as types)

const TAG = 'plugin-site-monitor-dashboard--plugin-spikes-100555';
...
export const tests: IPluginTestCase[] = [
    ...
    // pluginSpikes sets `legend` unconditionally — only `title` is gated by `mode`.
    { functionName: 'testPrepare', env: 'browser', params: [
        { input: { mode: 'full' },
          expected: { hasTitle: true, hasLegend: true, bodyHasWidget: true } },
        { input: { mode: 'simplified' },
          expected: { hasTitle: false, hasLegend: true, bodyHasWidget: true } },
    ]},
    ...
    { functionName: 'testPluginData', env: 'vscode', params: [
        { expected: { title: 'Spikes', svgHasSvgTag: true } },
    ]},
];
...
export async function testPrepare(testCase: { input: { mode: 'simplified' | 'full' }; expected: any }): Promise<string> {
    try {
        const el = await mount<PluginSpikes>(TAG, { scope: 'dashboard', mode: testCase.input.mode });
        await el.prepare();
        const result = {
...
export async function testHandleChange(testCase: { input: { selectedValue: string }; expected: any }): Promise<string> {
    try {
        const el = await mount<PluginSpikes>(TAG, { scope: 'dashboard' });
...
export async function testRenderRespectsScope(testCase: { input: { scope: string }; expected: any }): Promise<string> {
    try {
        const el = await mount<PluginSpikes>(TAG, { scope: testCase.input.scope as any });
