/// <mls fileReference="_100555_/l2/pluginSiteMonitorDashboard/pluginRegionalLatency.test.ts" enhancement="_blank" />

import { IPluginTestCase, mount, cleanup, compare, query, mountAndVerify } from '/_102027_/l2/plugins/pluginTestUtils.js';
import { pluginData, PluginRegionalLatency } from '/_100555_/l2/pluginSiteMonitorDashboard/pluginRegionalLatency.js';

import '/_100555_/l2/pluginSiteMonitorDashboard/pluginRegionalLatency.js'; // side-effect import: guarantees this module's top-level registration (e.g. @customElement) always runs, regardless of whether the named imports above survive compilation (TS elides imports used only as types)

const TAG = 'plugin-site-monitor-dashboard--plugin-regional-latency-100555';
...
export const tests: IPluginTestCase[] = [
    ...
    // pluginRegionalLatency keeps its chart data in `chartDataBar`, and renders the widget inside `.bar`.
    ...
    { functionName: 'testPluginData', env: 'vscode', params: [
        { expected: { title: 'Regional Latency', svgHasSvgTag: true } },
    ]},
];
...
export async function testPrepare(testCase: { input: { mode: 'simplified' | 'full' }; expected: any }): Promise<string> {
    try {
        const el = await mount<PluginRegionalLatency>(TAG, { scope: 'dashboard', mode: testCase.input.mode });
        await el.prepare();
        const result = {
            hasTitle: !!el.chartDataBar.title,
            hasLegend: !!el.chartDataBar.legend,
...
export async function testHandleChange(testCase: { input: { selectedValue: string }; expected: any }): Promise<string> {
    try {
        const el = await mount<PluginRegionalLatency>(TAG, { scope: 'dashboard' });
...
export async function testRenderRespectsScope(testCase: { input: { scope: string }; expected: any }): Promise<string> {
    try {
        const el = await mount<PluginRegionalLatency>(TAG, { scope: testCase.input.scope as any });
