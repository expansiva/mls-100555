...
import { pluginData, PluginExpenses } from '/_100555_/l2/pluginSiteMonitorDashboard/pluginExpenses.js';

import '/_100555_/l2/pluginSiteMonitorDashboard/pluginExpenses.js'; // side-effect import: guarantees this module's top-level registration (e.g. @customElement) always runs, regardless of whether the named imports above survive compilation (TS elides imports used only as types)
...
export async function testPrepare(testCase: { input: { mode: 'simplified' | 'full' }; expected: any }): Promise<string> {
    try {
        const el = await mount<PluginExpenses>(TAG, { scope: 'dashboard', mode: testCase.input.mode });
        await el.prepare();
        const result = {
...
export async function testHandleChange(testCase: { input: { selectedValue: string }; expected: any }): Promise<string> {
    try {
        const el = await mount<PluginExpenses>(TAG, { scope: 'dashboard' });
...
export async function testRenderRespectsScope(testCase: { input: { scope: string }; expected: any }): Promise<string> {
    try {
        const el = await mount<PluginExpenses>(TAG, { scope: testCase.input.scope as any });
