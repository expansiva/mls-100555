/// <mls fileReference="_100555_/l2/pluginExplore/pluginExploreListAddL1.test.ts" enhancement="_blank" />
...
import { ServiceListFilesAdd100555 } from '/_100555_/l2/pluginExplore/pluginExploreListAddL1.js';

import '/_100555_/l2/pluginExplore/pluginExploreListAddL1.js'; // side-effect import: guarantees this module's top-level registration (e.g. @customElement) always runs, regardless of whether the named imports above survive compilation (TS elides imports used only as types)

const TAG = 'plugin-explore--plugin-explore-list-add-l1-100555';
...
export const tests: IPluginTestCase[] = [
    ...
    // Real quirk: handleInputInput() validates `this.inputShortName.value` (the #iptShortName element
    // found via @query), not `e.target.value` — the event's target is only checked for truthiness.
    // Both params below feed a *different*, valid-looking value on the fake event target to prove the
    // queried DOM value — not the event — decides the outcome.
